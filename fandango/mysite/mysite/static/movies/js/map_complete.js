...
function initMap(config) {
    var mapContainer = d3.select('#mysite-map');
  ...
  var lines = svg.selectAll('path.graticule')
    ...
    d3.json("/static/movies/data/world-110m.json").then(function (world){
    ...
    config.render = function () {
        ...
        circle.enter()
            ...
            .attr("r", function(d) {return Math.max(3, Math.min(d.movies * 1, 30))})
